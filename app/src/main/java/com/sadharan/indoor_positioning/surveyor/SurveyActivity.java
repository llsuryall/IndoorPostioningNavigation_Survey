...
public class SurveyActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void initialize() {
        ...
        int temp_other = getIntent().getIntExtra(getString(R.string.floor_id_field), -1);
        if (temp >= 0) {
            surveyViewModel.block_id = temp;
        }
        if (temp_other >= 0) {
            surveyViewModel.floor_id = temp_other;
        }
        this.block_id = surveyViewModel.block_id;
        this.floor_id = surveyViewModel.floor_id;
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.save_button) {
            try {
                ...
                boolean successful = localSurveyDatabase.addDatapoint(new SurveyElement(floor_id, block_id, x_coordinate, y_coordinate, apSignalStrengths));
                if (successful) {

...
public class LocalSurveyDatabase extends SQLiteOpenHelper {
    private static final String BASE_URL = "http://somename-26741.portmap.io:26741/api/v1.0";
    //Database
        ...
        for (int i = 0; i < table_names.length; i++) {
            ...
            if (response.getBoolean("successful")) {
                ...
                for (int j = 0; j < items.length(); j++) {
                    ...
                    db.insert(table_names[i], null, row);
                }
            } else {
                return false;
            }
    ...
    public boolean addBlock(BlockElement blockElement) throws JSONException {
        ...
        if (response.getBoolean("successful")) {
            ...
            long row_id = db.insert(TABLE_BLOCK, null, block_row);
            return row_id >= 0;
        } else {
            return false;
        }
    ...
    public boolean addFloor(FloorElement floorElement) throws JSONException {
        ...
        if (response.getBoolean("successful")) {
            ...
            long row_id = db.insert(TABLE_FLOOR, null, floor_row);
            return row_id >= 0;
        } else {
            return false;
        }
    ...
    public boolean addDatapoint(SurveyElement surveyElement) throws JSONException {
        ...
        if (response.getBoolean("successful")) {
            SQLiteDatabase db = this.getWritableDatabase();
            ContentValues datapoint_row = new ContentValues();
            datapoint_row.put(COL_X_COORDINATE, surveyElement.x_coordinate);
            datapoint_row.put(COL_Y_COORDINATE, surveyElement.y_coordinate);
            datapoint_row.put(COL_FLOOR_ID, surveyElement.floor_id);
            datapoint_row.put(COL_BLOCK_ID, surveyElement.block_id);
            datapoint_row.put(COL_AP_SIGNAL_STRENGTHS, signalStrengthJSON.toString());
            long row_id = db.insert(TABLE_DATAPOINT, null, datapoint_row);
            return row_id >= 0;
        } else {
            return false;
        }
    ...
    private static class RequestThread extends Thread {
        ...
        public void sendPost() throws JSONException {
            ...
            try {
                httpURLConnection = (HttpURLConnection) new URL(BASE_URL + url).openConnection();
                byte[] requestDataBytes = requestData.toString().getBytes(StandardCharsets.UTF_8);
                httpURLConnection.setRequestMethod("POST");
                httpURLConnection.setRequestProperty("Content-Type", "application/json");
                httpURLConnection.setRequestProperty("Content-Length", "" + requestDataBytes.length);
                requestStream = new BufferedOutputStream(httpURLConnection.getOutputStream());

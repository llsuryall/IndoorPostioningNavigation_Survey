...
public class FloorCatalogueActivity extends AppCompatActivity implements View.OnClickListener, DialogInterface.OnClickListener, View.OnLongClickListener {
    ...
    private void initialize() {
        ...
        long temp = getIntent().getLongExtra(getString(R.string.block_id_field), -1);
        if (temp >= 0) {
            this.floorCatalogueViewModel.block_id = temp;
        }
        this.block_id = this.floorCatalogueViewModel.block_id;
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.addFloorButton) {
            ...
            surveyActivity.putExtra(getString(R.string.block_id_field), this.block_id);
    ...
    public void onClick(DialogInterface dialogInterface, int i) {
        ...
        boolean successful;
        try {
            successful = localSurveyDatabase.addFloor(new FloorElement(floor_id, block_id, floor_height, floor_width));
        } catch (Exception e) {
            successful = false;
        }
        if (successful) {
    ...
    */
    ...
    public boolean onLongClick(View view) {
        ...
        try {
            successful = this.localSurveyDatabase.deleteFloor(Integer.parseInt(((TextView) view.findViewById(R.id.floor_id)).getText().toString()), this.block_id);
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (successful) {
